...
from mfm.viz import render_sankey, render_kpis
...
with colA:
    ...
    library = st.multiselect("Pick from generic library", DEFAULT_LIBRARY, default=[
...
with colB:
    ...
    st.write("Keep it minimal — just enough to do mass balance and assign losses.")
    ...
    block["type"] = st.selectbox("Process type", [
        "intake", "prep", "cutting", "forming", "joining", "thermal", "surface", "assembly", "inspection", "packaging", "storage", "other"
    ], index=["intake","prep","cutting","forming","joining","thermal","surface","assembly","inspection","packaging","storage","other"].index(block["type"]), key=f"type_{idx}")

    st.markdown("**Primary material & throughput unit**")
    block["primary_material"] = st.text_input("Primary input material (free text)", value=block.get("primary_material", "Mild steel sheet 2mm"), key=f"mat_{idx}")
...
# -----------------------------
# Step 2: Data inputs (synthetic or uploads)
# -----------------------------
...
if demo_mode:
    ...
    st.info("Upload files (CSV/XLSX). AI will suggest what each file is and how to map columns.")
    ...
    if parsed:
        ...
        for name, df in parsed:
            ...
            dtype_confirm = st.selectbox(f"Confirm type for {name}", ["production_output", "material_purchases", "energy_site", "waste_summary"], index=["production_output","material_purchases","energy_site","waste_summary"].index(dtype))
...
# -----------------------------
# Step 3: Build model + compute + visualize
# -----------------------------
st.header("3) Generate the gate-to-gate material flow map")

if st.button("Generate flow map", type="primary", use_container_width=True):
    # Build a simple model object
    ...
    st.divider()
    st.subheader("Computed flows (table)")
